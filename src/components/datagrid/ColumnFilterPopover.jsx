...
export function ColumnFilterPanel({ columnKey, columnLabel, partition = 'all', currentModel, onApply, onClose }) {
  ...
  // Initialize from currentModel at mount time via useState initializer
  const [selectedValues, setSelectedValues] = useState(() => new Set(currentModel?.values || []));
  const [blanksSelected, setBlanksSelected] = useState(() => !!currentModel?.includeBlanks);
  ...
  const loadFilterValues = async (search = '') => {
    ...
    try {
      ...
      console.error('ColumnFilterPanel: error loading values', e);
    } finally {
      setLoading(false);
    }
  };
...
/**
 * ColumnFilterPopover — thin Radix Popover wrapper around ColumnFilterPanel.
