...
// Calculates ALL scenarios in a single call:
// - Auth once
// - Fetch persons once (voted=true)
// - Run all scenario calculations in-memory
// This avoids N×(auth + person pagination) which causes CPU timeout.

function calcScenario(scenario, votedPersons) {
    ...
    const totalPredictedVotes = partyResults.reduce((s, p) => s + p.predictedVotes, 0);
...
Deno.serve(async (req) => {
    try {
        ...
        // Fetch scenarios + dataset in parallel — once
        const [scenarios, datasets] = await Promise.all([
            base44.asServiceRole.entities.PredictionScenario.filter({ is_active: true }),
            base44.asServiceRole.entities.Dataset.filter({ status: 'active' }),
        ]);

        if (!scenarios?.length) return Response.json({ results: {} });
        if (!datasets?.length) return Response.json({ error: 'No active dataset' }, { status: 400 });

        const datasetId = datasets[0].id;

        // Fetch all voted persons — once, shared across all scenario calculations
        const votedPersons = [];
        let skip = 0;
        const batchSize = 500;
        while (true) {
            ...
            for (const p of batch) votedPersons.push(p);
            if (batch.length < batchSize) break;
            skip += batchSize;
        }

        // Calculate all scenarios in-memory (CPU only, no more DB calls)
        const results = {};
        for (const scenario of scenarios) {
            results[scenario.id] = calcScenario(scenario, votedPersons);
        }

        return Response.json({ results });
    } catch (error) {
        console.error('[scenarioCalculateAll] error:', error.message, error.stack);
